...
| 工具 | 哪个确定性执行器产生真实动作 |
| 人工治理规则集 | 哪些 Markdown 约束已由人批准、适用于什么范围和版本 |
| 证据 | 凭什么声称达到某一成熟度 |
| AI 审核裁决 | 哪个独立 AI 依据哪些具体规则允许、改写、阻断或报告规则缺口 |
| 规则缺口记录 | 哪条规则不足、当前对象如何安全处置、由谁异步修订规则 |
...
12. active Markdown 规则集是否有人工批准身份、scope、规则 ID 和内容 hash。
13. 普通内容是否由独立 AI 审核并逐项引用规则，而不是进入逐条人工审核队列。
