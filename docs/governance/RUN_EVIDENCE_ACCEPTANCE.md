...
`allow` 只表示当前 subject 满足已加载规则，不等于业务 Acceptance、不签发 Completion Claim，也不授予外部动作权限。`rewrite_required` 创建新 attempt 或 Run 并重新审核；达到上限转 blocked。`blocked` 阻断发布和后续副作用。`rule_gap` 按 `policies/rule-gap-case-contract.yaml` 创建 `rule_gap_case` 并阻断当前 subject，由人类异步维护规则，不进入逐条人工内容审核。
