...
```text
事前验收判据 → Run → Evidence → Acceptance Verdict → Completion Claim
```
...
Verdict 必须精确绑定对象版本/hash、需求基线、判据版本、Evidence hash、环境/输入类别、治理路由中的 overlay 状态快照、control set 和授权快照。任一 `required: true` 但 `enabled: false` 的叠加能力必须降低 claim ceiling 或阻断裁决。Claim 只能使用相同绑定且不能扩大 scope；任何绑定变化、未知引用或签名不一致都使 Verdict/Claim 失效。
