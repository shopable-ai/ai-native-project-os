...
本文件是 `proof_level` 唯一机器枚举、通用门禁公式、完成率算法和设计评分算法的唯一权威源。项目 owner、验收设计者、gate 实现者和评分审查者在定义证据目标、完成声明或评分快照前阅读；读完后应产出明确的 proof 目标、可执行门禁、锁定分母和可重算评分输入，而不是一个脱离 Evidence 的分数。
