...
| `human_decision` | 规则/事实/需求批准、例外与剩余风险接受、不可逆动作授权 | 人类身份、输入摘要、决定、时间、范围和有效期 |
| `external_system` | 受控边界之外的服务或设备 | 契约、超时、幂等、回执、失败和恢复语义 |

能够由确定性代码可靠完成的步骤，不默认交给模型。模型、Agent 和 Tool 是不同对象；Workflow 负责编排，不隐含无限权限。

`human_decision` 不承担日常内容质量审核。普通内容审核不得进入 `waiting_approval`；必须由 `ai_review` 节点输出 `allow/rewrite_required/blocked/rule_gap` 之一，并由确定性 gate 验证其机器契约。只有规则治理、事实/需求基线、例外、剩余风险和动作授权可以等待人工决定。
