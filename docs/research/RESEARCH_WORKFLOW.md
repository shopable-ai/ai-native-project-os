...
本文件是研究触发、优先级、研究包、结果升格和第三方采用门禁的唯一权威源。研究者、架构师和决策 owner 在关键未知、多个合理方案、高不可逆性或外部依赖进入决策路径时阅读；读完后应产出可审计研究包、Evidence、反方审查、研究 Verdict，以及必要时的候选 ADR，而不是未经批准的业务事实或正式依赖。
