...
`standard` 的中文显示和同名消歧只引用[术语权威](../governance/TERMINOLOGY.md#项目治理配置)；本文只维护基础治理配置和路由裁决的机器语义。
...
机器路由条件只由 `policies/project-governance-routing.yaml` 定义；路由裁决和逐 overlay 必填字段只由 `contracts/governance/route-decision-contract.yaml` 定义。本文解释语义，不维护第二份阈值。不得用业务关键词猜测。人工覆盖必须记录原路由、覆盖值、理由、批准者和有效期。
...
人工覆盖不能只写一句理由。`manual_override` 非空时必须保存原始路由、被覆盖字段和值、覆盖原因、批准者和到期时间；到期后重新计算路由，不能默认沿用旧覆盖。机器字段和不变量只由 `contracts/governance/route-decision-contract.yaml` 定义。
