...
| 输入 | 批准来源或存量资产、项目约束、未知项、当前需求基线和 Evidence。 |
| 输出 | 类型与治理路由、阶段位置、业务/工程链、批准功能需求、Spec/Task、执行记录、裁决与复盘去向。 |
...
```text
来源/存量恢复 → 项目类型 → 治理路由 → 生命周期阶段 → 研究 → 业务链路 → 能力树 → 功能树 → 功能级需求 → 人工批准与需求基线 → ADR → 工程设计 → Spec/Task → Workflow → Skill/Tool → Run → Evidence → Verdict → Claim → 复盘/升格
```
...
1. 把原始来源分成候选事实、假设和未知；只有责任人批准后才形成事实或需求。
...
7. AI 生成的 draft 不能自行升格。AI 先报告缺口、假设、可能错误和待确认项，再由人类批准需求版本并冻结 Requirement Baseline。

固定推导顺序是：`业务链路 → 能力树 → 功能树 → 功能级需求 → 人工批准与需求基线 → ADR → 工程设计 → Spec`。详细审查步骤见[人机协作需求设计工作流](REQUIREMENT_DESIGN_WORKFLOW.md)。
