...
| 输入 | 批准需求或存量资产、项目约束、未知项和当前 Evidence。 |
| 输出 | 类型与治理路由、阶段位置、业务/工程链、Spec/Task、执行记录、裁决与复盘去向。 |
...
```text
批准需求/存量恢复 → 项目类型 → 治理路由 → 生命周期阶段 → 研究 → 业务链路 → ADR → 工程链路 → Spec/Task → Workflow → Skill/Tool → Run → Evidence → Verdict → Claim → 复盘/升格
```
...
1. ADR 记录候选、取舍、Evidence、后果、退出条件和替代关系。
2. 工程链路根据已接受 ADR 分配人工、AI、代码与外部系统的职责，配套 I/O、失败、权限、兼容和验收判据。
3. Spec 是批准范围、计划、任务、验收与追溯的控制外壳；它不是运行节点。
4. **任务树从已批准的 Spec 与验收判据推导**。每个 Task 必须引用具体需求、判据、输入输出、依赖、失败路由和 Evidence 出口；禁止从能力树或头脑风暴直接生成无来源 Task。
