...
| `S2` | 从受控业务需求推导场景、链路、能力树、功能树和功能需求卡，执行 AI 自检、独立审核与 Decision Gate | 场景目录、业务链路、责任映射、能力/功能视图、功能需求卡、更新的受控需求基线 | S2 退出门：P0/P1 功能有意图、正常/异常/恢复、边界和验收方向；`policy_certified` 或 `human_signoff` 证据完整且阻断性 Unknown 清零 | 事实不足回 S1；意图、功能或恢复缺口留在 S2；不得从功能树直接跳到 Spec |
