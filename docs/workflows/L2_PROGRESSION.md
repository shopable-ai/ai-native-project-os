...
| `S0` | 明确目标、非目标、角色、成功指标，校验 original/approved intent，锁定需求分母、项目类型和治理路由初判 | 目标与范围、批准需求基线、owner/approver、route decision v1 | S0 退出门：目标负责人、批准意图、范围和验收方向明确；路由输入未知项有处置路径 | 目标、意图、范围或责任不清时重开 S0；商业目标变化也回到 S0 |
...
| `S2` | 从批准业务需求推导场景、链路、能力树、功能树和功能需求卡，执行 AI 自检与人工批准 | 场景目录、业务链路、责任映射、能力/功能视图、功能需求卡、更新的批准需求基线 | S2 退出门：P0/P1 功能有意图、正常/异常/恢复、边界、验收方向和人类批准 | 事实不足回 S1；意图、功能或恢复缺口留在 S2；不得从功能树直接跳到 Spec |
...
| `S4` | 只消费批准功能需求和 ADR，定义组件、工程链路、I/O、失败语义、权限、隔离、兼容和验收判据 | 组件边界、engineering chain、I/O contract、权限模型、acceptance criteria | S4 退出门：接缝、失败态、权限、兼容和回滚可验证，工程设计未扩大批准意图 | 架构前提错误回 S3；需求或意图错误回 S2/S0；契约不完整留在 S4 |
| `S5` | 从批准功能需求、当前 baseline、ADR、Spec 和验收倒推任务，建立 Workflow、Skill/Tool 和最小纵向实现 | Spec 五件套、Task 依赖图、Workflow、Skill/Tool、实现与 checkpoint 方案 | S5 退出门：Spec 精确绑定功能需求 version/content_hash 与批准需求基线；无孤儿任务，工作包可暂停、恢复和失败返回 | 未批准或过期需求 fail closed 并回 S2；契约不足回 S4；任务或实现问题留在 S5 |
